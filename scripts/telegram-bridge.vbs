...
flowName = GetConfigString(rootDir, "flows", "telegram_bridge", "telegram-bridge")
ps1Path = ExpandConfigPath(rootDir, GetConfigString(rootDir, "telegram_bridge_schedule", "powershell_wrapper", "{project_root}/scripts/telegram-bridge.ps1"))
commandTemplate = GetConfigString(rootDir, "telegram_bridge_schedule", "hidden_powershell_command_template", "powershell.exe -NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File {script}")

LogWrite rootDir, flowName, "INFO", "silent telegram bridge launcher started"

If Not fso.FileExists(ps1Path) Then
    LogWrite rootDir, flowName, "ERROR", "telegram bridge PowerShell wrapper missing path=" & ps1Path
    WScript.Quit 2
End If
...
If exitCode = 0 Then
    LogWrite rootDir, flowName, "OK", "silent telegram bridge launcher finished exit_code=0"
Else
    LogWrite rootDir, flowName, "ERROR", "silent telegram bridge launcher finished exit_code=" & CStr(exitCode)
End If
