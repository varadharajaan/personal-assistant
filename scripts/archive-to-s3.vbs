...
ps1Path = fso.BuildPath(scriptDir, "archive-to-s3.ps1")
flowName = GetConfigString(rootDir, "flows", "archive", "memory-archive")
commandTemplate = GetConfigString(rootDir, "archive_schedule", "hidden_powershell_command_template", "powershell.exe -NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File {script}")

LogWrite rootDir, flowName, "INFO", "silent archive launcher started"

If Not fso.FileExists(ps1Path) Then
    LogWrite rootDir, flowName, "ERROR", "archive PowerShell wrapper missing path=" & ps1Path
    WScript.Quit 2
End If
...
If exitCode = 0 Then
    LogWrite rootDir, flowName, "OK", "silent archive launcher finished exit_code=0"
Else
    LogWrite rootDir, flowName, "ERROR", "silent archive launcher finished exit_code=" & CStr(exitCode)
End If
