...
flowName = GetConfigString(rootDir, "flows", "openclaw_watchdog", "openclaw-watchdog")
ps1Path = ExpandConfigPath(rootDir, GetConfigString(rootDir, "openclaw_watchdog_schedule", "powershell_wrapper", "{project_root}/scripts/openclaw-watchdog.ps1"))
commandTemplate = GetConfigString(rootDir, "openclaw_watchdog_schedule", "hidden_powershell_command_template", "powershell.exe -NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File {script}")

LogWrite rootDir, flowName, "INFO", "silent openclaw watchdog launcher started"

If Not fso.FileExists(ps1Path) Then
    LogWrite rootDir, flowName, "ERROR", "openclaw watchdog PowerShell wrapper missing path=" & ps1Path
    WScript.Quit 2
End If
...
If exitCode = 0 Then
    LogWrite rootDir, flowName, "OK", "silent openclaw watchdog launcher finished exit_code=0"
Else
    LogWrite rootDir, flowName, "ERROR", "silent openclaw watchdog launcher finished exit_code=" & CStr(exitCode)
End If
